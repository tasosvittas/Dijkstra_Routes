...
def read_csv(filename):
    ...
    with open(filename, mode='r', newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            from_city = row[0].strip().upper()  # Normalize city names
            to_city = row[1].strip().upper()   # Normalize city names
            travel_mode = row[2]
            time = float(row[3])
...
#     raise ValueError(f"End city '{end_city}' not found in the graph.")
